...
During combat, each unit will manage their own 'Battle Points' or '**BP**'. These points are used to execute skills, where more powerful skills costing more BP. Units gain one BP at the start of their turn, and may choose to 'guard' or use a 'basic attack' (free actions) in order to accumulate BP. After choosing an action, the player may be prompted to choose a target if applicable.
